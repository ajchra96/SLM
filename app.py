...
if st.session_state.user is None:
    ...
    if col1.button("Login", use_container_width=True):
        try:
            ...
            # FIX: store simple dict instead of complex User object
            # This prevents serialization issues that were causing logout on form submits / reruns
    ...
    # ============ PAGE 3: PROFILE ============
        ...
        # FIX: use the variables we already extracted
        st.markdown(f"""
        **Email:** `{user_email}`
        **User ID:** `{user_id}`
        """)
